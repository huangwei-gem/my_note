...
:: 备份原始.gitignore文件
if exist ".gitignore" (
    copy .gitignore .gitignore.bak >nul
)

:: 将大文件添加到.gitignore
echo. >> .gitignore
echo # 大文件自动忽略 (自动添加于 %date% %time%) >> .gitignore
for /r %%f in (*) do (
    if exist "%%f" (
        for %%s in ("%%f") do (
...
if %errorlevel% equ 0 (
    echo 没有文件变更需要提交。
    goto restore_gitignore
)

:: 生成提交信息
echo.
set "COMMIT_MSG=Auto commit - %date:~-4%-%date:~3,2%-%date:~0,2% %time:~0,2%:%time:~3,2%:%time:~6,2%"
...
if exist ".gitignore.bak" (
    move /y .gitignore.bak .gitignore >nul
    echo 已恢复原始.gitignore文件
)
